...
namespace Maledictus.AStar
{
    public class Pathfinding : MonoBehaviour
    {
        ...
        public void FindPath(PathRequest request, Action<PathResult> callback)
        {
            ...
            {
                var openSet = new Heap<Node>(_gridManager.MaxChunkSize);
                ...
                while (openSet.Count > 0)
                {
                    ...
                    foreach (var neighbour in _gridManager.GetNeighbours(currentNode))
                    {
                        if (!neighbour.IsWalkable || closedSet.Contains(neighbour))
                            continue;

                        var newMovementCostToNeighbour = currentNode.G + _gridManager.GetDistance(currentNode, neighbour);

                        if (newMovementCostToNeighbour < neighbour.G || !openSet.Contains(neighbour))
                        {
                            neighbour.G = newMovementCostToNeighbour;
                            neighbour.H = _gridManager.GetDistance(neighbour, targetNode);
